...
*/
    ...
    List<Produit> produits = new ArrayList();
        String qry = "SELECT p.id ,c.libelle, p.nom,p.description,p.quantite,p.prix,p.image, (SELECT AVG(rating) from ratings where ratings.produit_id = p.id) as moyRating    FROM category c JOIN produit p ON c.id = p.categorys_id";
     try {
         Statement stm = cnx.createStatement();
